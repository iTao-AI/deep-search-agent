...
本仓库发布 backend、API、CLI、测试、文档、运维脚本和 React demo console。demo console 是面向 operator 的展示壳，用于说明现有 API、EvidenceLedger、review、verification 和 canonical result 边界。它保留静态 fallback，方便无后端时稳定演示；同时提供受控 Live Backend 模式，消费公开 run/result API。它不新增后端状态，也不成为业务事实源。
...
- React demo console 默认使用 Static Demo 模式，也可以通过受控 Live Backend 模式连接本机 backend。
