...
def generate_markdown(
    ...
    # 获取解析后的绝对路径字符串
    # 路径解析逻辑说明：
    # 1. full_input_path 由 `path` 和 `filename` 拼接而成。
    # 2. resolve_path 会根据 `session_dir` (会话根目录) 解析最终绝对路径。
    #
    # 场景示例 (假设 session_dir = "D:/Project/output/session_1"):
    # Case 1: path为空, filename="report.md"
    #   -> 结果: "D:/Project/output/session_1/report.md" (最常用，保存到会话根目录)
    # Case 2: path="sub_folder", filename="report.md"
    #   -> 结果: "D:/Project/output/session_1/sub_folder/report.md" (保存到会话子目录)
    # Case 3: path="/workspace", filename="report.md" (虚拟绝对路径)
    #   -> 结果: "D:/Project/output/session_1/report.md" (自动清洗虚拟前缀，确保安全)
    # Case 4: path是session_dir的子路径（合法子目录）
    #   Case 4.1: path="sub1/sub2", filename="report.md"
    #     -> 结果: "D:/Project/output/session_1/sub1/sub2/report.md" (保留子路径)
    #   Case 4.2: path="D:/Project/output/session_1/sub3", filename="report.md"
    #     -> 结果: "D:/Project/output/session_1/sub3/report.md" (绝对子路径直接使用)
